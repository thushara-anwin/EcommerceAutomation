...
def read_data(file,sheetname,row_num,colunm_num):
    workbook = openpyxl.load_workbook(file)
    sheet = workbook[sheetname]
    return (sheet.cell(row_num,colunm_num).value)


def write_data(file,sheetname,row_num,colunm_num,data):
    workbook = openpyxl.load_workbook(file)
    sheet = workbook[sheetname]
    sheet.cell(row_num,colunm_num).value= data
    workbook.save(file)
